...
//import quake from quakesmodel.js
import {Quake} from './QuakesModel.js';
...
export class QuakesController {
  ...
  async init() {
    ...
    await this.initPos();
    this.getQuakesByRadius(parseInt(localStorage.getItem('radius')));
  }
  async initPos() {
    ...
    if (this.position.lat === 0) {
     try {
        ...
        // if we get a position, set it in the model and in the view
        this.position = position;
        ...
        this.quakesView.setPosition(this.position);
  ...
  async getQuakesByRadius(radius = parseInt(localStorage.getItem('radius'))) {
  ...
  async getQuakeDetails(quakeId) {
    ...
    this.quakesView.renderQuakeDetails(quakeDetails, this.parentElement);
