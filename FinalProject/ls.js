...
//create a local storage function that will store the movies in the local storage when the user clicks the watch later button for each movie
export function storeMovie(movie){
    let movies;
    document.querySelector('.watchlaterBtn').addEventListener('click', () => {
        //add movie at a time to the local storage when the watch later button is clicked
        ...
        movies.push(movie);
        //set the local storage to the movies array for each movie
...
//create function when the user clicks on the watchlater they will be redirected to the watch later page
